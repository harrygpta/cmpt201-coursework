...
data from? How can you tell in the code? From the standard input (keyboard),
looked at read(STDIN_FILENO, buf, BUF_SIZE)
4. How does the client program end? How can you tell that in the code?
   It ends when standard input sends EOF (e.g. Ctrl-D). You can tell because the
while loop ends when read returns 0 or 1.
*/

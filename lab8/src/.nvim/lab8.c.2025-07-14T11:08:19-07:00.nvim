...
// Hints:
// - Use counter_thread_func for
// pthread_create(..) and modify as
// needed.
// - Store the threads and their
// arguments so you can manage them later
// - Initialize and pass the mutex to
// protect the critical sections (Ta
...
static count_map_t count_words_parallel(word_t *words, size_t num_words) {
  ...
  for (size_t i = 0; i < THREAD_COUNT; i++) {
    ...
    free(threads_args[i]); // Cleanup the
                           // args struct
  }
...
// Takes in an array of words of size
// num_words and returns a hash table
// where the key is the word and the
...
int main(void) {
  ...
  // Task 2: Replace this function call
  // with the parallelized version.
  // word_map = count_words_seq(words_in,
  // words_in_len);
...
static void *counter_thread_func(void *param) {
  // Call count_words_in_chunk with the
